...
GitHub-hosted Windows의 엄격한 cadence 검증은 moving `windows-latest` alias가
아닌 고정 `windows-2022` image에서 실행합니다. 일반 CI와 Release packaging은
`windows-latest`를 계속 사용해 최신 Windows 호환성도 별도로 확인합니다.
GitHub-hosted runner는 짧은 확인이나 4시간 이하 검증에 적합합니다.
...
- `top_cadence_due_lateness_samples`: 가장 늦은 10개 submit의 대상, due/submit monotonic 시각, 실행 경과, wall-clock 시각. 공유 runner 일시 정지와 애플리케이션 병목을 구분하는 원인 분석 증거
...
공유 GitHub-hosted VM이 일시적으로 deschedule되어도 cadence 임계값을 완화하거나
누락 due를 다시 anchor하지 않습니다. 고정 image에서도 같은 host-wide pause가 반복되면
해당 실행은 그대로 실패 처리하고 전용 self-hosted Windows runner에서 증거를 다시 수집합니다.
